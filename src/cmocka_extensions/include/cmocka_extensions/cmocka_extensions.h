...
#define TEST_CASE(__name)                                                      \
  cmocka_unit_test_setup_teardown(__name, __name##Setup, __name##Teardown)
...
#define RUN_TEST_SUITE(__testsuite, __name)                                    \
  cmocka_run_group_tests(__testsuite, __name##Setup, __name##Teardown)
 ...
 */
static inline void assert_double_equal(const double left, const double right,
                                       const double epsilon) { // NOLINT
  double absLeft;
  double absRight;
  double largest;
  double relDiff;
...
#define will_set_parameter(METHOD, PARAM_NAME, VALUE)                          \
